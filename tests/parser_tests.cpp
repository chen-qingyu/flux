...
{
    const auto model = flux::test_support::parse_model(std::filesystem::path("data") / "any_resource_minimal.bpmn");

    const auto& start = model.node("Event_start");
    const auto& task = model.node("Task_service");
...
{
    const auto model = flux::test_support::parse_model(std::filesystem::path("data") / "all_resources_minimal.bpmn");
...
{
    const auto model = flux::test_support::parse_model(std::filesystem::path("data") / "fifo_queue_minimal.bpmn");
